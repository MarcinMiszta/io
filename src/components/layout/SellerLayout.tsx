...
import { LayoutDashboard, Map, MapPin, CreditCard, History, LogOut, User, Store, Menu, X } from 'lucide-react';
...
export const SellerLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const navItems = [
        { label: 'Pulpit', to: '/seller', icon: <LayoutDashboard size={20} />, end: true },
        { label: 'Mapa Targowiska', to: '/seller/map', icon: <Map size={20} /> },
        { label: 'Moje Rezerwacje', to: '/seller/reservations', icon: <MapPin size={20} /> },
        { label: 'Dostępne Stanowiska', to: '/seller/stands', icon: <Store size={20} /> },
        { label: 'Płatności', to: '/seller/payments', icon: <CreditCard size={20} /> },
        { label: 'Historia', to: '/seller/history', icon: <History size={20} /> },
    ];

    const sidebarColor = '#D97706';
    const sidebarDark = '#B45309';

    const SidebarContent = () => (
        <>
            <div style={{ padding: '24px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                <h1 style={{ fontSize: '20px', fontWeight: 'bold', color: 'white' }}>Panel Klienta</h1>
    ...
    return (
        <div className="layout-container">
            ...
            <div className="mobile-header" style={{ backgroundColor: sidebarColor }}>
                <span className="mobile-header-title" style={{ color: 'white' }}>Panel Klienta</span>
